...
class MeterIdentificationClusterDateStruct(
  val year: UByte?,
  val month: UByte?,
  val day: UByte?,
  val dayOfWeek: UByte?,
  ...
  override fun toString(): String = buildString {
    append("MeterIdentificationClusterDateStruct {\n")
    append("\tyear : $year\n")
    append("\tmonth : $month\n")
    append("\tday : $day\n")
  ...
  companion object {
    ...
    fun fromTlv(tlvTag: Tag, tlvReader: TlvReader): MeterIdentificationClusterDateStruct {
      tlvReader.enterStructure(tlvTag)
      val year =
        if (!tlvReader.isNull()) {
          tlvReader.getUByte(ContextSpecificTag(TAG_YEAR))
        } else {
      ...
      val month =
        if (!tlvReader.isNull()) {
          tlvReader.getUByte(ContextSpecificTag(TAG_MONTH))
        } else {
      ...
      val day =
        if (!tlvReader.isNull()) {
          tlvReader.getUByte(ContextSpecificTag(TAG_DAY))
        } else {
      ...
      val dayOfWeek =
        if (!tlvReader.isNull()) {
          tlvReader.getUByte(ContextSpecificTag(TAG_DAY_OF_WEEK))
        } else {
      ...
      return MeterIdentificationClusterDateStruct(year, month, day, dayOfWeek)
    }
  }
}
